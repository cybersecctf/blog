...
<body>
    <h1>ConfusionthroughSubstitution- cryptohack</h1>
...
Challenge files:
  - <a href="https://cybersecctf.github.io/blog/2024/practice/cryptohack/symmenticcryptography/ConfusionthroughSubstitution/sbox.py">sbox.py</a>
    ...
    <ol>
        we run this code for sub two matrix and get flag
<pre>
import blog
def sub_bytes(s, sbox):
    sub_state = [[s_box[byte] for byte in sublist] for sublist in state]
    # Substitute each value in the state matrix based on the inv_s_box
    inv_sub_state = [[inv_s_box[byte] for byte in sublist] for sublist in state]
    return    inv_sub_state
def solve(state,s_box):
    inv_sub_state=sub_bytes(state,s_box)
    return blog.solveup("matrix","matrixtobyte",inv_sub_state)
...
<br>
    ...
    <h2>Conclusion</h2>
    <p>this is a very   easy chanllenge for Substitutions two matrix on hex</p>
</body>
</html>
